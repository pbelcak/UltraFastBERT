...
def main_training_process(cfg, setup):
    ...
    for step, batch in enumerate(dataloader, initial_step + 1):
        ...
        time_progress = (time.time() - wallclock_timer) / 3600 / cfg.budget
        step_progress = step / cfg.train.steps
        progress = max(time_progress, step_progress)

        # Collect stats and print to console and upload to wandb
        if step % cfg.impl.print_loss_every_nth_step == 0:
            loss_vals, train_time = collect_stats(step, loss_vals, train_time, stats, model_engine, dataloader, cfg, progress)
...
def collect_stats(step, loss_vals, train_time, stats, model_engine, dataloader, cfg, progress):
    stats["step"] += [step]
